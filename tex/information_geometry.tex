...
\end{equation}
%
We define an optimal estimator to be unbiased and have minimal
variance, \ie a covariance matrix equal to the Fisher information.  In
this way, the Fisher information matrix $I_{ij}$ encodes the maximal
...
some point $\boldtheta_a$, see the left panel of
...
$\boldtheta_a$ in this tangent space,
%
\begin{equation}
...
\begin{figure}
  ...
  \begin{tikzpicture}[
    ...
    % global space
    \foreach \t in {0,1,...,599}{%
      \draw[color=dark-red!70!white] (-3,{-3+0.01*\t},{-\curvaturefactor*(9 + (-3+0.01*\t)^2)})
      -- (-3,{-3+0.01*(\t+1)},{-\curvaturefactor*(9 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] (-2,{-3+0.01*\t},{-\curvaturefactor*(4 + (-3+0.01*\t)^2)})
      -- (-2,{-3+0.01*(\t+1)},{-\curvaturefactor*(4 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] (-1,{-3+0.01*\t},{-\curvaturefactor*(1 + (-3+0.01*\t)^2)})
      -- (-1,{-3+0.01*(\t+1)},{-\curvaturefactor*(1 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] (0,{-3+0.01*\t},{-\curvaturefactor*(0 + (-3+0.01*\t)^2)})
      -- (0,{-3+0.01*(\t+1)},{-\curvaturefactor*(0 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] (1,{-3+0.01*\t},{-\curvaturefactor*(1 + (-3+0.01*\t)^2)})
      -- (1,{-3+0.01*(\t+1)},{-\curvaturefactor*(1 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] (2,{-3+0.01*\t},{-\curvaturefactor*(4 + (-3+0.01*\t)^2)})
      -- (2,{-3+0.01*(\t+1)},{-\curvaturefactor*(4 + (-3+0.01*(\t+1))^2)});
      ...
      \draw[color=dark-red!70!white] ({-3+0.01*\t},-3,{-\curvaturefactor*(9 + (-3+0.01*\t)^2)})
      -- ({-3+0.01*(\t+1)},-3,{-\curvaturefactor*(9 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] ({-3+0.01*\t},-2,{-\curvaturefactor*(4 + (-3+0.01*\t)^2)})
      -- ({-3+0.01*(\t+1)},-2,{-\curvaturefactor*(4 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] ({-3+0.01*\t},-1,{-\curvaturefactor*(1 + (-3+0.01*\t)^2)})
      -- ({-3+0.01*(\t+1)},-1,{-\curvaturefactor*(1 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] ({-3+0.01*\t},0,{-\curvaturefactor*(0 + (-3+0.01*\t)^2)})
      -- ({-3+0.01*(\t+1)},0,{-\curvaturefactor*(0 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] ({-3+0.01*\t},1,{-\curvaturefactor*(1 + (-3+0.01*\t)^2)})
      -- ({-3+0.01*(\t+1)},1,{-\curvaturefactor*(1 + (-3+0.01*(\t+1))^2)});
      \draw[color=dark-red!70!white] ({-3+0.01*\t},2,{-\curvaturefactor*(4 + (-3+0.01*\t)^2)})
      -- ({-3+0.01*(\t+1)},2,{-\curvaturefactor*(4 + (-3+0.01*(\t+1))^2)});
    ...
    \foreach \t in {0,1,...,99}{%
      ...
      \draw[thick,color=dark-red] ({-0.02*\t}, {-0.01*\t - 0.00000 * \t^2}, {-\curvaturefactor*((-0.02*\t)^2+(-0.01*\t - 0.00000 * \t^2)^2)})
      -- ({-0.02*(\t+1)}, {-0.01*(\t+1) - 0.00000 * (\t+1)^2}, {-\curvaturefactor*((-0.02*(\t+1))^2+(-0.01*(\t+1) - 0.00000 * (\t+1)^2)^2)});
    }
    
    % Tangent space
    \draw[color=dark-blue!70!white, fill=dark-blue!10!white, fill opacity=0.2] (3,-3,0) -- (3,3,0) -- (-3,3,0) -- (-3,-3,0) -- cycle;
...
by a metric in the physical subspace. Instead of minimizing the
distance itself, we can therefore absorb the minimization procedure
...
genuine theory parameters and $t$ parameterises the tangent through
...
The most obvious application for this tool are systematic and theory
...
For this first demonstration we neglect systematic und theory
...
\begin{figure}
  ...
  \caption{Trade-off between signal purity and information in the WBF
    ...
    50~\gev$. The coloured dots show three examplaric selections.}
...
\begin{figure}
  ...
  \caption{Error ellipses defined by the Fisher information in the
    ...
    indicate distances of $d = 1,2,\dots,5$. In grey we show example
...
analysis focusses on the question which of the phase-space
...
supressed with an appropriate event selection~\cite{Kling:2012up}.
...
\thpyramid{th_aa_global}{fig:information_th_global_distances} {Error
...
mode. Our setup is exactly as in \autoref{sec:information_wbf_taus},
...
requires parameterized smearing functions and an interpolation of the
...
measurement. The aim of our approach is to minimize the covariance
matrix of estimators, which the Cram\'er-Rao bound links to the Fisher
information. Alternatively one can maximize the power of hypothesis
...
of arbitrary dimensionionality.
...
\autoref{sec:information_wbf_taus} we sample parameter points
...
information-based analysis, utilizing the convenient properties for
...
channel as described in \autoref{sec:information_wbf_taus} and sample
...
Information geometry can be used to optimize measurements of any
continous theory parameters at the LHC. Following the Cram\'er-Rao
...
high-dimensional parameter spaces since it requires no discretization
...
arbitrary high-energy physics processeses based on Monte-Carlo
...
discrimination power.  Under idealized conditions, traditional
...
data and under idealized conditions, these distributions are only
