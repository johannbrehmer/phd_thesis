...
on the theory space. This is the cornerstone of information
...
distance measure between parameter points. First, we can define a
\emph{local distance} in the tangent space at some point
$\boldtheta_a$,
%
\begin{equation}
...
Goying beyond the tangent space, \emph{global distances} can be
defined along geodesics,
%
\begin{equation}
...
parameter. Global distances are particularly useful to track the
effect of higher powers in the theory parameters, as we show in
\autoref{sec:information_eft}.
...
{Error ellipses defined by the Fisher information in the WBF
  ...
  $d(\boldtheta,\mathbf{0})$, where in each panel the $\theta^i$ not
...
independent of EFT basis rotations.
...
products of the Higgs are mostly sensitive to the decay kinematics. We
...
by five observables characterizing the decay kinematics:
%
\begin{itemize}
...
the Higgs mass. To overcome this disadvantage, decay effects require a
very large production cross section; it remains to be seen if the
$gg \to h \to 4\ell$ channel will suffice.
