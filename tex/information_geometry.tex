...
$h \to W^+W^- \to (\ell^+ \nu) (\ell^- \overbar{\nu})$ is a $2 \to 6$
process even at parton level, made even more complicated by additional
...
Analyses of collider signatures with \emph{traditional methods}
typically begin with simple selection cuts on standard kinematic
observables such as energies, momenta, angles, or invariant
masses. Background contributions are estimated with simulational or
data-driven methods. The final statistical analysis often relies on
total event counts or on histograms of kinematic observables.  These
techniques are motivated by physical arguments, all their steps are
transparent, and they are relatively easy to reproduce, even for
theorists. They work well for simple signatures, such as a resonance
peak on top of a smooth background, as in the discovery of the
$gg \to h \to \gamma \gamma$
signal~\cite{Aad:2012tfa,Chatrchyan:2012xdj}. This approach requires a
careful tailoring to each problem, and does not scale well with the
complexity of the theory questions or of the kinematics of the processes.
...
parton shower to take into account the information encoded in the jet
...
A second class of complex statistical tools belongs to the category of
\emph{likelihood-free inference}~\cite{cranmer:nips2016}. These
techniques are mostly based on (supervised) machine learning:
%sophisticated interpolation techniques
flexible nonlinear functions, for example boosted decision trees or
deep neural networks, are `trained' to describe patterns in a set of
...
these samples can be based on data or on full-fledged
Monte-Carlo simulations with a parton shower and a detailed detector
simulation. New ideas are developed at an impressive
...
tracking~\cite{Brehmer:ghost_probability, Aad:2014yva} to the analysis
of jet substructure~\cite{Cogan:2014oua, Baldi:2014pta,
  deOliveira:2015xxd, Almeida:2015jua, Baldi:2016fql, Guest:2016iqz,
  Komiske:2016rsd, Kasieczka:2017nvn, Louppe:2017ipp}, to the
discrimination between signal and background
hypotheses~\cite{Baldi:2014kfa, Searcy:2015apa, Santos:2016kno,
  Alves:2016htj}, and finally to the statistical testing of model
hypotheses~\cite{Buckley:2011kc, Bornhauser:2013aya,
...
often convoluted and not necessarily intuitive, and it is not always
clear which physical structures these `black boxes' are sensitive to.
...
%
An efficient estimation of the sensitivity of measurements to new
physics signatures that does not require a full simulation of the
analysis chain can help plan experiments.
%
Moreover, the design of event selections and analysis strategies
requires clearly defined guidelines.

We present a novel approach to tackle these problems based on
information geometry~\cite{efron1975, amari1982, amari2000joho}. It is
...
any discretisation of the model hypothesis. These properties are
particularly convenient for measurements of Higgs properties in terms of
a large number of effective operators.
...
quantities outlined above, and use them to answer practical questions:
%
\begin{itemize}
...
\item How well can we measure model parameters based on individual
  ...
  high-dimensional phase space?
    %
\item Which role do non-linear effects in the
...
\newparagraph
%
The Fisher information is commonly used in various fields, for
instance in cosmology~\cite{Jaranowski:1994xd, 2012MNRAS.423.3445S,
  2012JCAP09009W, Bringmann:2016axu}, but only rarely in particle
physics~\cite{Sirunyan:2017idq}. To the best of our knowledge, most of
the tools presented in Reference~\cite{Brehmer:2016nyr} and in this
chapter are innovative at least for this field. Since the publication
of Reference~\cite{Brehmer:2016nyr}, these and similar ideas have
received more attention~\cite{Ferreira:2017ymn, Edwards:2017mnf}.
...
\end{equation}
%
or for more than one parameter the covariance matrix
%
\begin{equation}
...
\end{equation}
%
where we implicitly sum over repeated indices $i$, $j$. For an
unbiased estimator, \ie for $\overbar {\boldtheta} = \boldtheta_0$,
the Cram\'er-Rao bound takes on the simple form
%
\begin{equation}
  \cov \left[ \hat{\boldtheta} \middle| \boldtheta_0 \right]_{ij}
...
measurement. The Fisher information matrix $I_{ij}$ hence
...
variance is an unbiased estimator with minimum variance, but it does
not reach the lower bound given by
\autoref{eq:information_cramer_rao_unbiased}. Even if an efficient
...
shown that under certain regularity assumptions\footnote{One condition
  ...
  of couplings, leading to the same predictions for cross sections as
  the SM. But these considerations are irrelevant for the analysis of
  small deformations from the SM with effective operators in this
  thesis.}, the \emph{maximum-likelihood estimator} defined by
%
\begin{equation}
  \hat{\boldtheta}_{\text{MLE}} (\boldx) = \argmax_\boldtheta f(\boldx | \boldtheta)
\end{equation}
%
is asymptotically unbiased and efficient,
%saturates the Cram\'er-Rao bound,
\ie in the limit of large sample size it leads to the minimum
covariance matrix defined by the inverse Fisher information
...
given by the Fisher information for problems with small event
numbers.
...
information does not depend on arbitrary basis choices: it is independent of
the parametrisation of the observables $\boldx$, and transforms
...
manifold given by the theory space. This is the underpinning of
information geometry~\cite{efron1975, amari1982, amari2000joho}. In
this field, methods from differential geometry are used to analyse the
structure of the probability distributions, or the relation between
experiment and models.  Interpreted as a metric, the Fisher
information defines infinitesimal distances
%
\begin{equation}
...
at some point $\boldtheta_a$.
%, see \autoref{fig:information_geometry_illustration}.
We usually consider the distance between $\boldtheta_a$ itself and
some other point $\boldtheta_b$ in this tangent space,
%
\begin{equation}
...
\end{equation}
%
with $\boldtheta(s_a) = \boldtheta_a$ and
$\boldtheta(s_b) = \boldtheta_b$.  The shortest path $\boldtheta(s)$
...
parameter points that does not require choosing one of them to evaluate the
...
information for the measurement of theory parameters $\boldtheta$ in
such an experiment reads
%
\begin{equation}
...
The most straightforward way to add kinematic features is to measure a
differential cross section or histogram of a kinematic observable $v$,
for instance a transverse momentum, energy, invariant mass, or
...
\begin{equation}
  ...
  = L \, \sum_{\text{bins } b} \pder {\sigma_b (\boldtheta)} {\theta_i}  \, \frac 1 {\sigma_b (\boldtheta)} \, \pder {\sigma_b (\boldtheta)} {\theta_j} \,.
...
defines which kinematic observables provide probes of which
directions in theory space. In practice, this can be used as a
guideline for which distributions to measure and publish.
...
including all correlations and without relying on a choice of
observables or binning, we use an \emph{extended likelihood} ansatz,
also known as marked Poisson process.  The observables in LHC physics
consist of a total number of events $n$, each of which has some
high-dimensional kinematic properties $\boldx_i$. The probability
...
\begin{equation}
  ...
  = L \, \pder {\sigma (\boldtheta)} {\theta_i}  \, \frac 1 {\sigma (\boldtheta)} \, \pder {\sigma (\boldtheta)} {\theta_j}  \;
    + L \, \sigma(\boldtheta) \, 
...
single-event likelihood functions $f^{(1)} (x |\boldtheta)$ are
complex expressions given by a sum of intricate amplitudes. Instead of
...
events:\footnote{In its simplest form, this step implicitly assumes
  that we can experimentally distinguish between all phase-space
  points. Resolution effects and undetected particles can be taken
  into account with suitable transfer
  functions~\cite{Artoisenet:2008zz, Mattelaer:2011ywa,
    Mertens:2014iya} or with a simple smearing as discussed in
  \autoref{sec:information_algorithm}.}
%
\begin{equation}
...
analysis compared to a hypothetical ideal multivariate analysis.
...
phase-space region for measurements and should drive the design of
...
discussed here. The former answers the question of how well we can
...
the events in a certain slice of the phase space.
...
corresponds to the best estimate of the nuisance parameters. The
...
some parameters of interest, for instance the theory parameters,
without explicit dependence on the nuisance parameters. In a Bayesian
approach, one can assign a prior probability distribution to the
nuisance parameters and integrate (`marginalise') over them. Instead,
we follow a conservative method that is consistent with a frequentist
definition of statistics and does not require the choice of a
prior. Loosely speaking, we calculate the minimum sensitivity to
theory parameters for any value of the nuisance parameters within
their constraints (\ie for nuisance parameters that are `allowed to
float'). This concept of `profiling' over nuisance parameters is well
established for instance for hypothesis tests based on the likelihood
...
\end{equation}
%
The contours of the local distance $(\boldtheta_a , \boldzero)$ define
an ellipsoid around this point. The contours of the profiled distance
are just the parallel projection of this ellipsoid onto the subspace
...
To calculate the profiled Fisher information, we start from the
ellipsoid
%
\begin{equation}
...
parameters of interest and $t$ parametrises the tangent
...
This can be easily generalised to multiple nuisance parameters. For a
Fisher information matrix of the block form
%
\begin{equation}
  I_{ij} = \twomat {I^{\text{theory}}} {(I^{\text{mix}})^T} {I^{\text{mix}}} {I^{\text{nuisance}}} \,,
  \label{eq:information_profiling_full_information_general}
\end{equation}
%
the profiled Fisher information matrix reads~\cite{Edwards:2017mnf}
%
\begin{equation}
  I^{\text{profiled}} = I^{\text{theory}} - (I^{\text{mix}})^T \, (I^{\text{nuisance}})^{-1} \, I^{\text{mix}} \,. 
  \label{eq:information_profiling_profiled_information_general}
\end{equation}
%
In addition to the geometric explanation given above, this result can
be understood from the Cram\'er-Rao bound: the upper left block of the
inverse of \autoref{eq:information_profiling_full_information_general}
is equal to the inverse of
\autoref{eq:information_profiling_profiled_information_general}.
...
\emph{new physics reach}
%
\begin{equation}
  \frac {\Lambda} {\sqrt{f}} = \frac v {\sqrt{\Delta \theta_{\text{min}}}} =  v \; I^{1/4} \,.
...
curvature of the theory manifold, therefore provides an intuitive
measure of the role of $\ord{1/\Lambda^4}$ contributions.

\newparagraph
%
Different bases for effective operators are physically equivalent, and
the directions in theory space defined by the basis operators
$\ope{i}$ do not have a special meaning. Any statistical tool that
singles out individual directions or parameter points in the model
space can introduce a bias towards a specific operator base. In the
worst case, it can be blind to effects on diagonal directions, \eg
...
traces, and determinants of the Fisher information are invariant under basis
...
$\boldtheta_n$ using the same phase-space grid.\footnote{An
  alternative tool for this step, which supports a variety of transfer
  functions to describe the detector response, is
...
WBF case.  We then normalise the Higgs rates to the recommendations of
the LHC Higgs Cross Section Working Group for the total cross
sections~\cite{deFlorian:2016spz}, calculating the effect of the
...
we replace the propagator of the corresponding $s$-channel particle
with the square root of a smearing function, for instance a Gaussian
or double Gaussian with parameters fitted to ATLAS and CMS
results. Other detector effects are not included in the first studies
presented here. However, a more realistic description of the
experimental resolution can easily be achieved by including a complete
set of transfer functions, similar to the usual approach for the
matrix element method~\cite{Artoisenet:2008zz, Mattelaer:2011ywa,
  Mertens:2014iya}.

\newparagraph
%
The second step is based on the fact that the differential cross
section can be decomposed into a finite set of components,
...
multiplicatively to vertices, as is often the case for effective field
...
coupling $g$ in the SM and new physics amplitude proportional to the parameter $\theta$. The
...
\begin{table}[!b]
  \setlength{\tabcolsep}{5pt} % otherwise the table is too wide for the page, default is 6pt
...
$\boldtheta$, without having to rerun the Monte-Carlo simulation.

Derivatives with respect to the theory parameters can easily be
...
dimensionality. It can be calculated based on the full phase
...
% \begin{align}
%   C_{ij}(\boldtheta)
%   \equiv E \left[ (\hat{\theta}_i-\overbar{g}_i) (\hat{\theta}_j-\overbar{g}_j)  \middle | \boldtheta \right] \,,
...
operators in Higgs measurements. Three processes are considered: Higgs
production in weak boson fusion with a decay into tau pairs or four
leptons, and Higgs production with a single top.
...
$\tau \tau \to (\ell \nu \nu) (j j\nu)$, and assume that the di-tau
system can be reconstructed with a reasonable resolution for
$m_{\tau\tau}$, for instance with the collinear
approximation~\cite{Rainwater:1998kj, Plehn:1999xi, Plehn:2009nd}. We
take the experimental mass resolution into account by smearing the
$m_{\tau \tau}$ distributions. The smearing functions and their
parameters are estimated from Figure~1a of
...
\end{align}
%
to include as much of the phase space as possible.
...
\begin{equation}
  ...
  \varepsilon^\text{CJV}_{\text{ggF $H$}} = 0.14 \,,
\end{equation}
%
...
absorbed into a single universal efficiency factor $\varepsilon$. We
calculate the Fisher information for $pp$ collisions at
$\sqrt{s} = 13~\tev$ for an integrated luminosity times universal
efficiencies of
%
\begin{equation}
...
$\boldtheta = \boldzero$ as well as on the two-dimensional planes in
parameter space where all but two operators are zero. All in all, we
calculate Fisher information matrices at approximately 6000 parameter
points.
...
\begin{figure}[!b]
  \includegraphics[width=0.49 \textwidth,clip=true,trim=0 0.0cm 0 0.3cm]{fig/information/wbf_tautau_3d_low}%
  \includegraphics[width=0.49 \textwidth,clip=true,trim=0 0.0cm 0 0.3cm]{fig/information/wbf_tautau_3d_high}%
...
this process, and do not affect the measurement of the other
...
measurement in this process in the absence of new physics.
...
\begin{figure}
  ...
  \caption{Distribution of the differential Fisher information in the
    ...
    momentum (right). We also show the normalised rates for the SM
    signal (solid black) and the QCD $Zjj$ background (dotted
    grey). The dashed blue lines show the effect of an exaggerated new
    physics benchmark $f_{W} \, v^2 / \Lambda^2 = 0.5$ on the WBF
    signal. The last bin is an overflow bin.}
...
determinants. The resulting distributions of differential information
over typical kinematic variables are shown in
...
\begin{figure}
  ...
  \caption{Distribution of the differential Fisher information in the
    ...
    right). We also show the normalised rates for the SM signal (solid
    black) and the QCD $Zjj$ background (dotted grey). The dashed blue
    lines show the effect of $f_{W} \, v^2 / \Lambda^2 = 0.5$ on the
    WBF signal, except in the bottom right panel, where we show
...
dimension-six operators include derivatives, leading to an amplitude
increasing with the momentum transfer through the gauge-Higgs vertex. This
...
% Note that these distributions also quantify how
...
backgrounds thus discard a sizeable fraction of the information on
...
consider a class of typical WBF cuts
%
\begin{equation}
...
%
\begin{equation}
  p_{T,j}^{\vphantom{\text{max}}} < p_{T,j}^{\text{max}} \,,
...
$h \to \tau \tau$ channel for different cuts of the form of
...
the Fisher information in this plane in model space. Following
...
require any validity cuts for the EFT to work. However, for smaller
Wilson coefficients $0.25 \lesssim f \lesssim 1$, the scale hierarchy
is less clear, confirming the discussion in
...
\item the separation in azimuthal angle between the two jets, now
  ...
  $\smash{\Delta \phi_{jj} = \phi_{j_{\eta < 0}} - \phi_{j_{\eta >0}}}$, with
  bin size $2 \pi / 20$;
%
...
\begin{figure}
  ...
  \includegraphics[width=0.49 \textwidth,clip=true,trim=0 0.6cm 0 0.0cm]{fig/information/wbf_tautau_histos_contours_likelihoodcut}
  \caption{Information in distributions in the WBF $h \to \tau \tau$
    ...
    (yellow). The $\theta_i$ not shown are set to zero. Left: based on
    the kinematic WBF cuts in
    \autoref{eq:information_wbf_tautau_wbfcuts}. Right: based on the
    matrix-element-based selection in
    \autoref{eq:information_wbf_tautau_likelihoodcuts}.}
  \label{fig:information_wbf_tautau_histograms_contours}
\end{figure}

In the left panel of
\autoref{fig:information_wbf_tautau_histograms_contours}, we compare
...
straightforward measure of the information in the distributions that is
...
\newparagraph
%
In the light of the large loss of information due to the WBF cuts in
\autoref{eq:information_wbf_tautau_wbfcuts}, we repeat this comparison
with an alternative matrix-element-based event selection. Instead of
cutting on standard kinematic observables, we select all events in
`signal-like' phase-space regions, defined as those with a larger
expected SM WBF rate than the combined expected background rates,
%
\begin{equation}
...
\autoref{fig:information_wbf_tautau_histograms_contours} and in
...
\begin{figure}[!b]
  \includegraphics[width=0.33 \textwidth,clip,trim=0.3cm 0.6cm 0.05cm 0]{fig/information/wbf_4l_geometry_fphi2_fw}%
  \includegraphics[width=0.33 \textwidth,clip,trim=0.3cm 0.6cm 0.05cm 0]{fig/information/wbf_4l_geometry_fphi2_fww}%
  \includegraphics[width=0.33 \textwidth,clip,trim=0.3cm 0.6cm 0.05cm 0]{fig/information/wbf_4l_geometry_fww_fw}%
...
production vertex (fixing the decay vertex to the SM value) and
on the decay vertex (where the production is fixed to the SM
structure). We find that only the sensitivity to $\ope{\phi,2}$ profits
from effects in the decay vertex. Since this operator rescales all Higgs
...
entirely from effects on the Higgs production.
...
small. Our results show that this is not compensated by the complex
$h \to 4\ell$ kinematics.
...
production with $h \to 4\ell$ has a rate that is approximately 13
times larger than the WBF process~\cite{deFlorian:2016spz}. Since the
decay kinematics is the same as in the process studied here and the
Fisher information scales linearly with the number of events, we can
provide a rough estimate of the information in this process by scaling
the decay-only Fisher information with this factor 13. We find that
even with the increased rate from production in gluon fusion, the
$h \to 4 \ell$ channel is not as sensitive to Higgs-gauge operators as
the WBF production process. Of course, this simple estimate ignores
any differences in the backgrounds, efficiencies, systematic or theory
uncertainties, and Higgs kinematics between the two production
processes.
...
by observables characterising the decay
...
\item the smaller invariant mass of the reconstructed $\ell^+ \ell^-$, $m_{Z_2}$;
  % 
\item the angle $\smash{\cos \theta_1 = \hat{p}_{\ell^-_1} \cdot \hat{p}_{Z_2}
  \big|_{Z_1}}$ with spatial unit vectors $\hat{p}$ defined in the $Z_1$ system;
  % 
\item analogously $\cos \theta_2$; and
  % 
\item the angle
  $\smash{\cos \Phi = ( \hat{p}_{\ell^-_1} \times \hat{p}_{\ell^+_1} )
...
necessity of removing backgrounds efficiently, the combined
...
our previous results. This shows again how the sensitivity of the
...
five-flavour scheme. For our toy example we include only one of the
...
\begin{figure}[!b]
  ...
  \caption{Feynman diagrams for $t$-channel Higgs plus single top
    production with $h \to \gamma \gamma$ and a hadronic top
    decay. The red dots show the Higgs interactions modified by the
    dimension-six operators considered in our analysis.}
...
For the total Fisher information at the SM, we find
%
\begin{equation}
  I_{ij} (\boldzero) =
\begin{pmatrix*}[r]
...
\begin{figure}[!b]
  \includegraphics[width=0.33 \textwidth,clip,trim=0.3cm 0.6cm 0.05cm 0]{fig/information/th_geometry_fphi2_fw}%
  \includegraphics[width=0.33 \textwidth,clip,trim=0.3cm 0.6cm 0.05cm 0]{fig/information/th_geometry_fphi2_ft}%
  \includegraphics[width=0.33 \textwidth,clip,trim=0.3cm 0.6cm 0.05cm 0]{fig/information/th_geometry_ft_fw}%
...
\begin{figure}
  ...
  \caption{Distribution of the differential SM Fisher information in
    ...
    system. We also show the normalised rates for the SM signal (solid
    black) and the single-top background (dotted grey). The dashed
    blue lines show the effect of $f_{t} \, v^2 / \Lambda^2 = 0.2$ on
    the $th$ signal. In the right panel, the last bin is an overflow
    bin.}
...
\begin{figure}
  ...
  \caption{Distribution of the differential SM Fisher information in
    ...
    the top decay products (bottom). We also show the normalised rates
    for the SM signal (solid black) and the single-top background
    (dotted grey). The dashed blue lines show the effect of
    $f_{t} \, v^2 / \Lambda^2 = 0.2$ on the $th$ signal. The last bins
    are overflow bins where applicable.}
...
The next question is again how much of this maximal information is
contained in various kinematic distributions. A histogram-based
analysis is only feasible with a reasonable signal-to-background
ratio, so we require
%
\begin{equation}
  p_{T,j_1} > 50~\gev \,, \quad 
...
operators. In \autoref{fig:information_th_histograms_contours}
...
predictions. In \autoref{sec:information_systematics} we demonstrate
how such effects change our
results. \autoref{sec:information_comparison} finally compares the
Fisher information to two other statistical tools, the well-known
...
The precision of most Higgs measurements at the LHC are ultimately
limited by systematic and theory
uncertainties~\cite{deFlorian:2016spz, Khachatryan:2016vau}. The
expected differential signal and background cross sections are
not exactly known, but depend on nuisance parameters $\boldnu$. In
\autoref{sec:information_nuisance} we showed how these can be
incorporated in our approach, and defined a profiled Fisher
information.
...
dimension-six operators in WBF Higgs production with
$h \to \tau \tau$. Our setup is the same as in
\autoref{sec:information_wbf_tautau}, except that we assign a $5\%$ or
$10\%$ Gaussian uncertainty on the overall signal rate, representing
for instance missing higher orders, parton density function
uncertainties, or errors on the
efficiencies. \autoref{fig:information_wbf_tautau_systematics}
demonstrates how profiling over these nuisance parameters
significantly reduces the information on $\ope{\phi,2}$, which only
rescales the total signal rate. The other operators are mostly
constrained from kinematic shapes and much less sensitive to such a
systematic uncertainty on the overall normalisation.

In \autoref{fig:information_wbf_tautau_systematics_comparison} we show
how the information in various distributions is affected by such a
systematic error, in complete analogy to
...
lemma, the best test statistic is the likelihood ratio between the two
hypotheses. These two objects are designed for different questions:
the likelihood ratio compares two discrete hypotheses, while the
Fisher information describes continuous parameter spaces of arbitrary
dimensionality. Nevertheless, it is instructive to compare these
tools.
...
likelihood ratio to the SM.

As shown in \autoref{fig:information_wbf_tautau_llr}, the local and
especially the global distances are almost exactly equal to the square
root of the expected likelihood ratio, with small differences only
becoming visible around the $3 \sigma$ level.  This demonstrates that
different statistical tools probe the same physics and can be chosen
based on practical considerations. In particular, the conclusions from
an information-based analysis, utilising the convenient properties of
the Fisher information for high-dimensional theory spaces, also apply
to exclusion limits based on the log likelihood ratio.
...
\autoref{sec:information_formalism}, assigns a structure to the
parameter space of a theory. It is, interestingly, also possible to define
...
with which model parameters can be estimated. This object is
well-suited to high-dimensional parameter spaces since it requires no
discretisation of the tested hypotheses, does not depend on arbitrary
basis choices, and can simultaneously encode the maximum sensitivity
to all directions in model space in a single matrix. These properties
make information geometry particularly useful for the analysis of
effective field theories. Moreover, the Fisher information is additive
between different measurements and between different phase-space
regions within the same process, making it trivial to discuss the
combination of different channels. Understood as a metric on the
theory space, it provides an intuitive geometric interpretation of
...
The Fisher information can also be calculated differentially to
understand how the discriminating power is distributed over phase
space, helping define optimal event selection. Alternatively, we
can calculate the information in individual distributions of kinematic
observables rather than the full high-dimensional kinematics. This
defines the most powerful observables, and lets us compare the power
of simple histogram-based analyses to methods based on the matrix
elements or machine-learning techniques. All of these instruments can
be useful to improve measurement strategies.

An interesting feature of the geometric interpretation is that the
curvature of the Riemannian manifold defined by the Fisher information
provides a handle on the square of dimension-six contributions. As
discussed in \autoref{sec:validity_squares}, under certain assumptions
this lets us analyse the convergence of the EFT expansion in
$1/\Lambda$.
...
decay into tau pairs.  As expected, a large fraction of the
...
$550~\gev$, while an optimal multivariate analysis may be able to
...
profiling procedure for the Fisher information, we can pay particular
attention to the question which signatures of $CP$ violation are
genuine, \ie cannot be caused by any $CP$-conserving new
physics. Finally, while we focused on Higgs physics in terms of
effective field theories in this thesis, these tools can be applied to
any other measurement of continuous parameters in perturbative LHC
processes.
