...
%
\begin{equation}
  \frac {\Lambda_{\text{max}}} {\sqrt{f}} = \frac v {\sqrt{\Delta \theta_{\text{min}}}} =  v \; I(\boldzero)^{1/4} \,.
\end{equation}
%
The maximal probed energy scale $\Lambda_{\text{max}}$ depends on the
...
For the event generation we use \toolfont{MadMax}~\cite{Plehn:2013paa,
...
$\boldtheta_n$ using the same phase-space grid. An alternative option
that supports a variety of transfer functions to describe the detector
response is \toolfont{MadWeight}~\cite{Artoisenet:2008zz,
  Mattelaer:2011ywa, Mertens:2014iya}.
...
The event generation is based on our own
\toolfont{FeynRules}~\cite{Alloul:2013bka} model file of the relevant
dimension-six operators, which does not truncate operator effects at
$\ord{1/\Lambda^2}$. \toolfont{MadMax} requires fixed renormalization
and factorization scales, which we set following
Reference~\cite{deFlorian:2016spz}. To keep the calculation times
manageable, we restrict the weak boson fusion production process to
the dominant sub-process with initial-state $u$ and $d$ quarks.  We
then normalize the Higgs rates to the LHC HXS WG recommendations for
the total cross section~\cite{deFlorian:2016spz}, calculating the
effect of the different acceptance regions with
\toolfont{MadGraph~5}. Background processes are simply rescaled to
\toolfont{MadGraph} predictions.
%
\comment{To here}
