...
dependence. The second complication comes from the Higgs production
channels and decay modes, some of which have a high-dimensional phase
space. For example, Higgs production in weak boson fusion with a decay
$h \to W^+W^- \to (\ell^+ \nu) (\ell^- \overbar{\nu})$ is a $2 \to 6$
process even at parton level, and only made more complicated by
additional QCD radiation. This defines a large number of kinematic
observables, and it is often not obvious which of them carry
information on the theory parameters of interest. The relation between
the high-dimensional parameter space and the potentially
high-dimensional phase space is at parton level encoded in the
amplitudes or Feynman diagrams, of which there can easily be hundreds
for a given process.

Traditional analysis methods typically begin with simple selection
cuts on standard kinematic observables such as energies, momenta,
angles, or invariant masses. This is often followed by the estimation
of background contributions with a mixture of simulational and
data-driven methods. The final statistical analysis typically relies
on total event counts or on histograms of kinematic observables.
These techniques are very intuitive and all steps are
...
signal~\cite{Aad:2012tfa,Chatrchyan:2012xdj}. But this approach faces
challenges when the theory questions or the processes are more
complicated.

At the other end of the spectrum, measurement strategies based on
high-level statistical tools are gaining gaining popularity among the
LHC collaborations~\cite{cranmer:nips2016}.
%
\comment{To here}



% Traditional analysis methods: require certain objects, cutting on kinematic observables, counting events or fitting histograms of standard observables. Work well to discover signal peak in clean channel, but not necessarily for Higgs property measurements.

% Modern abnalysis methods, multivariate stuff, matrix element methods, optimal observables. Potentially better, but often opaque. Many tools designed for the comparison of two discrete hypotheses.

% With these capabilities it is increasingly important to be able to characterise the information in observables, and to be have guidelines for the design of event selections and analysis strategies

% We develop an approach based on information geometry. Intrinsically designed for continous parameter spaces of arbitrary dimensionality. Unlike many other statistical techniques, no need for discretization of hypothesis.

% Fisher information and Cramer-Rao bound

% Our algorithm

% Questions we can answer for LHC processes

% First application: CP-even operators in Higgs signatures for WBF Higgs production (4l, tau tau) and t + H

% Second application: CP violation; disentangling genuine signatures from CP violation from signatures of CPV dim-six operators that can also be caused by CP-even new physics

% Demonstrate how our approach allows us to treat systematic uncertainties; provide context by comparing it to log likelihood ratio and Hellinger distance (?)

% commonly used in gravitational wave physics. but not in hep (exception: CMS study). Also mention 

% While we demonstrate our approach for Higgs observables and dim-6 operators, our algorithm works fine for any process described by matrix elements and theories with continous parameters
...
% \autoref{eq:information_covariant_transformation} then allows us to
