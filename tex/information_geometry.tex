...
\end{equation}
%
\comment{Include calculation here, or in appendix?}
...
We compare the two tools for WBF Higgs production in the $\tau \tau$
mode, following the setup described in
\autoref{sec:information_wbf_taus}. We sample parameter points
$\boldtheta$ in the $\ope{W}$-$\ope{WW}$ plane. For each of these
points we calculate the local and global distance from the SM defined
by the Fisher information, as well as the expected log-likelihood
ratio
%
\begin{align}
...
\comment{Mention that our results to not include systematics and
  theory uncertainties; link to Chapter 3.}
