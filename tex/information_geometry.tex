...
\begin{align}
  ...
  \label{eq:information_information_simple_example_information}
\end{align}
%
...
\autoref{eq:information_simple_example_information} non-singular and remove all
...
\end{align}
%
The Cram\'er-Rao bound in \autoref{eq:information_cramer_rao_unbiased} then states
that any unbiased estimator has
...
$v_{\text{min}} \leq v < v_{\text{max}}$. In the
limit of many small bins that span the entire kinematically allowed
range for $v$, \autoref{eq:information_information_histo} gives the
information in the distribution of $v$.
...
We calculate the quantities sketched in the previous sections with a
combination of existing Monte-Carlo generators and our own algorithm,
which we call \toolfont{MadFisher}. 
%
\comment{To here}

\comment{First explain three basic ingredients to Fisher information
  calculation: 1.~MC calculation of delta sigma for benchmark points;
  2.~Morphing setup to get delta sigma and its derivatives at
  arbitrary theta; 3.~calculate Fisher information from expressions
  given above.}

In a first step, we generate event samples for a number of benchmark
parameters $\boldtheta_s$ with \toolfont{MadMax}~\cite{Plehn:2013paa,
  Kling:2016lay}. This add-on to
\toolfont{MadGraph~5}~\cite{Alwall:2014hca} allows us to
simultaneously calculate the differential rates $\Delta \sigma$ for
different parameter points $\boldtheta_s$ using the same phase-space
grid.

A morphing technique then allows us to calculate the differential
cross sections and their derivatives at arbitrary positions in
parameter space~\cite{morphing}. The effect of $\ope{\phi,2}$ and
systematic rate uncertainties (see \autoref{sec:systematics}) is taken
into account analytically. The contributions from the other operators
are decomposed into a number of basis components and can be exactly
reconstructed from a set of simulated benchmark points.

The derivatives with respect to the theory parameters can
...
\end{equation}
%
with a small vector $\varepsilon_i$ in $i$-direction.

We can then easily calculate the Fisher information according to
Equations~\eqref{eq:information_information_rate},
\eqref{eq:information_information_histo}, and
\eqref{eq:information_information_from_events}.

Finally, global distances as in \autoref{eq:information_distances} are
calculated in analogy to free fall in general relativity: a starting
point and a set of directions in parameter space define the initial
conditions, from which we numerically calculate distances along curves
defined by the geodesic equation.



% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% % First, our setup builds on the fact that the information matrices in
% % Equations~\eqref{eq:information_information_rate},
% % \eqref{eq:information_information_histo}, and
% % \eqref{eq:information_information_from_events} can all be calculated
% % from a set of events, each defined by a phase-space point $\boldx$ and
% % a differential cross section $\Delta \sigma (\boldtheta)$, where the
% % parameters $\boldtheta$ now implicitly include potential nuisance
% % parameters. 
...
% We can then easily calculate the Fisher information according to
% \autoref{eq:information_information_from_events}. Finally, global distances as in
% \autoref{eq:information_distances} are calculated in analogy to free fall in
...
As can be seen from \autoref{eq:information_information_from_events}, the
...
given in \autoref{eq:information_global_distances}. The difference between
...
Following \autoref{eq:information_cramer_rao_precision}, the Fisher information in a given direction
...
\autoref{eq:information_wbf_tautau_acceptance_cuts} and applying the CJV
...
Following \autoref{eq:information_def_wilson}, our model space is spanned by five
dimensionless parameters
%
\begin{align}
  \boldtheta = \frac {v^2} {\Lambda^2}  \fivevec {f_{\phi,2}} {f_W} {f_{WW}} {f_{B}}  {f_{BB}}  \,.
\label{eq:information_wilson_space_wbf}
\end{align}
%
...
defined in \autoref{eq:information_distances} for slices of parameter space in
