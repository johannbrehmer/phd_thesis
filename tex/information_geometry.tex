...
are identical to those in that article. These are supplemented with a
...
generally either rely on the comparison of two discrete hypotheses or
are designed for the measurement of one direction in theory
space. Applying these methods to high-dimensional theory spaces, such
as Higgs effective field theory, often requires a discretisation of
the theory space, which can be computationally expensive. In addition,
care has to be taken to avoid dependencies on EFT basis choices.
...
simulations. We can calculate the total information, which
...
multivariate analysis may be able to probe scales up to $700~\gev$
