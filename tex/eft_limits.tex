...
\autoref{sec:validity_practical_questions}, and give our
...
well as long as there is a large separation between the experimentally
...
necessary to separate the signal from the QCD backgrounds often require
...
\end{equation}
%
depending on the process, observable, amount of data collected, and
analysis methods.
...
\end{equation}
%
Here $g$ is a typical coupling of the underlying theory and
...
size of the couplings of the underlying physics. For very weakly
coupled theories, $g^2 \ll 1$, only new physics models with new
particles at or below the electroweak scale can leave measurable
signatures in Higgs observables, and the EFT approach is not
justified. For truly strongly coupled theories,
$1 < g \lesssim 4 \pi$, new physics scenarios up to
$\Lambda \lesssim 5~\tev$ are relevant, and the EFT expansion
converges flawlessly. In fact, the EFT approach to Higgs observables
has largely been motivated by the desire to describe models based on a
strongly interacting electroweak symmetry
...
scale hierarchy, and the EFT expansion in $1/\Lambda$ does not
converge fast. But this does not mean that an analysis of LHC data in
terms of a truncated dimension-six Lagrangian cannot be
...
between full models and their effective approximations. We select
...
model for a range of Higgs observables. In this way we analyse the
convergence of the EFT expansion in $1/\Lambda$ by comparing the
dimension-six operators not just to the dimension-eight terms, but to
the infinite number of operators corresponding to the full model.
...
\item Which observables are correctly described by the dimension-six
  model? 
%
\item Where does the EFT description break down, and does this pose a
  problem for LHC analyses?
%
...
amplitudes from dimension-six operators should be included in
calculations while dimension-eight operators are neglected. Both
...
physics~\cite{Alwall:2007ed, Hagiwara:2009wt, Englert:2012xt,
  Brehmer:2014pka, Brehmer:2014dnr}. Similar points were discussed for
...
have to define how we construct the effective theories.  Matching the
...
\begin{equation}
  \frac {f_i} {m^2} \, \ope{x}
  = \frac  {f_i}  {M^2 \pm g v^2}\, \ope{i}
  = \left( \frac  {f_i}  {M^2} \mp \frac { f_i g v^2}{M^4} \right) \ope{i}
  + \ord{1/M^6} \,.
\end{equation}
...
and represents the conventional approach for the linear Higgs EFT. The
dimension-six operators are formulated in terms of the doublet $\phi$
and based on the assumption $\Lambda \gg v$, so the EFT should be
matched to the full theory in the unbroken phase of the electroweak
...
phase. This is a somewhat subjective criterion that depends on the model
...
Let us try to understand $v$-improved matching from a different
...
\begin{align}
  ...
  %
  &\equiv \lgr{SM}
    + \sum_i \frac {f_i^{(6)}} {\Lambda^{2}} \, \ope{i}^{(6)} 
    + \sum_{d=8}^\infty \sum_j \frac {f_j^{(d)}} {\Lambda^{d-4}} \, \ope{j}^{(d)} \notag \\
  %
  &= \lgr{SM}
  + \sum_i \frac {f_i^{(6)}} {\Lambda^{2}} \, \ope{i}^{(6)} 
  + \sum_i \sum_{n=1}^\infty \frac {f_i^{(6+2n)}} {\Lambda^{2+2n}} \, \left( \phisq \right)^n \, \ope{i}^{(6)}
...
corresponds to replacing $\phisq \to v^2 / 2$ in (part of) the second
sum\footnote{The argument is slightly more complicated if additional
  ...
  factor to the definition of the $v$-improved Wilson coefficient in
  \autoref{eq:validity_v-improvement_resummation}.}:
%
\begin{equation}
  \lgr{$v$-improved dim-6} = \lgr{SM}
  + \sum_i
  \underbrace{  \left[  \frac {f_i^{(6)}} {\Lambda^2}  + \sum_{n=1}^\infty \frac {f_i^{(6+2n)}} {\Lambda^{2+2n}} \, \left( \frac {v^2} {2} \right)^n \right] }_{\displaystyle (f_i^{(6)} / \Lambda^2)_{\text{$v$-improved}}}
  \ope{i}^{(6)} \,.
...
coefficients of the dimension-six operators. The remaining parts of
the operators in \autoref{eq:validity_dim8_phisq_dim6} (the terms from
$\phisq \to vh + h^2/2$) and the higher-order operators of an entirely
different type cannot be absorbed into the dimension-six Lagrangian in
this way.

Let us illustrate this with the example of
\autoref{sec:validity_matching_ambiguities}, where a heavy particle
with mass scale $M$ in the unbroken electroweak phase has a coupling $g$ to Higgs
doublets. Matching the theory in the unbroken phase, we
...
simply a natural choice of parameters to describe a model. We thus expect that
...
absence of a large enough scale separation, we can test if the
$v$-improved matching enhances the validity of the dimension-six
Lagrangian.
...
Effects in SM-like Higgs couplings will be parametrised with the
...
production process on its own and with a Higgs decay to four leptons or to photons,
%
\begin{align}
  g g &\to h \to 4 \ell \,,\notag \\
...
require\footnote{For a definition of all kinematic quantities, see
  Appendix~\ref{sec:appendix_pheno}.}
%
\begin{equation}
...
\end{equation}
%
to avoid large contributions from the $Z$ peak and from
bremsstrahlung.
...
\end{equation}
%
where we require typical WBF cuts
%
\begin{align}
  p_{T,j} &> 20 \ \gev \,, &
 \Delta \eta_{jj} &> 3.6 \,, &
...
neither Higgs decays nor kinematic cuts are expected to affect the
questions we analyse, hence we leave them out.
...
$\alpha_{\text{ew}}$, which is sufficient given the size of new physics
...
implementations in \toolfont{FeynRules}~\cite{Alloul:2013bka,
  Degrande:2011ua}.  For the dimension-six predictions we resort to a
modified version of the \toolfont{HEL} model
file~\cite{Alloul:2013naa}.
...
samples using appropriate tree-level couplings. Next, we compute the
...
$\sqrt{s} = 13$~\tev. We use the \toolfont{CTEQ6L} parton density
function~\cite{Pumplin:2002vw} and the default dynamical choices of
the factorisation and renormalisation scale implemented in
\toolfont{MadGraph~5}. For this broad survey of EFT validity we limit
...
Model adds a real scalar singlet $S$~\cite{Silveira:1985rk,
...
the singlet $\lambda_3(\phisq)S^2$, which is responsible for
...
points in \autoref{tbl:validity_singlet_benchmarks}, with heavy
Higgses ranging from $200$ to $1000$~\gev. The first three scenarios
are in agreement with all experimental and theoretical constraints at
the time of publication of Reference~\cite{Brehmer:2015rna}. This
...
sufficient to capture the coupling shifts, even though a significant
...
below this scale. This can be seen
...
full theory well. But this agreement deteriorates already at
moderately larger energies, with no significant difference between
default and $v$-improved matching, and clearly breaks down towards the
$H$ pole.
...
the singlet model are of the form
$\smash{(\phisq)^n_{\vphantom{i}} \ope{i}^{(6)}}$. After EWSB they can
be resummed into the Wilson coefficients of the dimension-six
operators as given by \autoref{eq:validity_v-improvement_resummation}.
...
transfer, even for the $v$-improved matching.
%
% Of course, the effective theory by definition does not
...
\begin{multline}
  V(\phi_1,\phi_2) =
  m^2_{11}\,\phi_1^\dagger\phi_1^{\vphantom{\dagger}}
  + m^2_{22}\,\phi_2^\dagger\phi_2^{\vphantom{\dagger}}
  + \frac{\lambda_1}{2} \, (\phi_1^\dagger\phi_1^{\vphantom{\dagger}})^2
  + \frac{\lambda_2}{2} \, (\phi_2^\dagger\phi_2^{\vphantom{\dagger}})^2
  + \lambda_3 \, (\phi_1^\dagger\phi_1^{\vphantom{\dagger}})\,(\phi_2^\dagger\phi_2^{\vphantom{\dagger}})  \\
  + \lambda_4 \, |\phi_1^\dagger\,\phi_2^{\vphantom{\dagger}}|^2
  + \left[ - m^2_{12}\,\phi_1^\dagger\phi_2^{\vphantom{\dagger}}
    + \frac{\lambda_5}{2} \, (\phi_1^\dagger\phi_2^{\vphantom{\dagger}})^2 + \hc \right] \,.
...
extension, these coupling modifications are not universal, reflecting
the more flexible flavour structure of the model. Second, there are
three new heavy resonances $H^0$, $A^0$, and $H^\pm$, which should
...
\newparagraph
%
Two aspects complicate the decoupling in the 2HDM: first, `delayed
decoupling' effects appear after electroweak symmetry
...
dimension-six effect, but can be enhanced by large values of
$\tan \beta$, thus delaying the decoupling of the heavy 2HDM states in
the Yukawa sector.

Second, unlike in the MSSM, the Higgs self-couplings $\lambda_i$ and
$m_{12}$ are not bounded from above. Terms of the form
$\lambda_i v^2$, potentially enhanced with factors of $\tan \beta$,
contribute to the masses of the heavy Higgs bosons and to the
...
\eqref{eq:validity_2hdm_matching2} remain unchanged.\footnote{An
  alternative definition of the $v$-improved Wilson coefficients could
  be based on the physical mixing angles $\alpha$ and $\beta$. Similar
  to the singlet model, in this way we could ensure that the
  tree-level Higgs-fermion couplings agree exactly between the full
  and the effective model. Here we refrain from such a fine-tuned
  matching prescription to demonstrate how a simple redefinition of
  $\Lambda$ can improve the performance of the dimension-six model.}
The two matching schemes can exhibit significant differences in the
2HDM since the pseudoscalar mass
$m^2_{A^0} = m_{12}^2/(\sin\beta\cos\beta) -\lambda_5\,v^2$ does not
coincide with $\Lambda_{\text{default}}$ over large parts of the
parameter space.
...
time of publication of Reference~\cite{Brehmer:2015rna}, implemented
with the help of \toolfont{2HDMC}~\cite{Eriksson:2009ws},
...
between $1\sigma$ and $2\sigma$ in the Higgs couplings measurements. 
...
%
\begin{enumerate}
\item[D1] \emph{Moderate decoupling}: emphasises large Higgs
  coupling shifts, with up to $2\sigma$ deviations from the LHC
  constraints. Additional Higgs masses around $250 \dots 350$~GeV can
  leave visible imprints.
%
\item[D2] \emph{Supersymmetric}: reproduces the characteristic mass
...
not reproduce the coupling patterns of the full model. We conclude
that an EFT matched to the 2HDM in the unbroken electroweak phase is
essentially useless, and we have to rely on $v$-improved matching. For
simplicity, we will from now on leave out the results based on the
default matching, which only confirm these initial results.

The $v$-improved matching captures most of the coupling
...
leads to up to $10 \%$ deviations, in agreement with the results for
the coupling shifts.
...
\begin{figure}[t]
  ...
  \caption[Kinematic distributions in the 2HDM]{Selected kinematic
    distributions in 2HDM benchmark D1, comparing the full model to
    the EFT based on $v$-improved matching. Left: $m_{\tau \tau}$
    distribution in the $gg \to \tau^+ \tau^-$ channel. Right:
    off-shell behaviour of the process $gg \to h^0 \to \gamma \gamma$,
    only taking into account the Higgs diagrams. At
    $m_{\gamma \gamma} \gtrsim 2 m_{H^\pm} = 700\ \gev$, the charged
    Higgs threshold is visible. The shaded error bands give the
...
The 2HDM discussion leads us to a similar conclusion as
...
angle $\theta_{\tilde{t}}$. We provide a detailed description of the
model setup in Appendix~\ref{sec:appendix_models_stops}.
...
Lorentz structures. The Yukawa couplings do not change at one-loop level
...
\end{equation}
%
This shift can be sizeable only for relatively low stop and sbottom masses
...
The stop loops generate a number of operators~\cite{Henning:2014wua,
  Drozd:2015kva, Drozd:2015rsp},
%
\begingroup%
\allowdisplaybreaks%
\begin{align}
...
EFT become larger, the validity of the dimension-six model worsens, and
...
that of $\ope{\phi,2}$, which corresponds to a universal rescaling of
...
$\smash{V_{\mu\nu}^a = D_\mu^{\vphantom{a}} V_{\nu}^a - D_\nu^{\vphantom{a}} V_{\mu}^a}$ and where
$\smash{D_\mu^{\vphantom{a}} V_\nu^a = \partial_\mu^{\vphantom{a}} V_\nu^a+ g_V \varepsilon^{abc}_{\vphantom{\mu}} V^b_\mu
V_\nu^c}$.
...
physics. Loop-induced contributions are further suppressed and we
neglect them here.
...
We study a set of benchmark points defined in
...
\begin{table}
  \setlength{\tabcolsep}{5.5pt} % otherwise the table is too wide for the page, default is 6pt
...
in the extreme scenarios T1 and T2 we find significant deviations. 
...
$hWW$ vertex~\cite{Eboli:2000ze, Plehn:2001nj, Hankele:2006ma,
  Hagiwara:2009wt, Englert:2012xt, Buckley:2014fqa, Brehmer:2014pka, Brehmer:2014dnr}. 
...
constraints on the vector triplet model can be determined directly in
the full model or indirectly via EFT measurements, and how this is
...
precisely, the momentum transfer can easily be calculated, for
instance with the Mandelstam variables $s$, $t$, and $u$. For WBF
Higgs production, both the definition and the measurement of the
momentum transfer are more involved. We have to take into
account that this is a $2 \to 3$ process even without adding a Higgs
decay, and that the charges of the initial
and final partons cannot be determined experimentally.
...
their interference is negligible. This allows us to distinguish `$W$-like'
...
Experimentally we can neither distinguish between the jets from
up-type and down-type quarks, nor can we determine the charges of the
initial-state partons. The true momentum transfer $q$ therefore cannot
be calculated and we have to resort to an experimentally accessible
proxy. The transverse momenta of the two tagging jets, $p_{T,j1}$ and
$p_{T,j2}$, are two good candidates. As can be seen from the Feynman
diagram in \autoref{fig:foundations_production_diag}, these two jets
directly recoil against the intermediate vector bosons, so
...
correlations of the virtuality $q$ with the transverse momentum of the
leading tagging jet and with the transverse momentum of the Higgs
boson. Both observables are visibly correlated with the momentum
transfer, but the correspondence is particularly clear for the jet
$p_T$.
...
Higgs production. More precisely, we ask which observable $x$
%
\begin{enumerate}
\item can isolate phase-space regions with interesting NP signatures with
  a cut $x > x_{\text{min}}$, and simultaneously
\item ensures the EFT validity with a cut $x < x_{\text{max}}$.
\end{enumerate}
%
We compare the observables
%
\begin{equation}
  x \in \left\{  q, \, p_{T,j1}, \, p_{T,j2}, \, p_{T,h} \right\} \,.
\end{equation}
%
...
\begin{figure}
  ...
  \caption{Trade-off between sensitivity to new physics signatures and
    EFT validity in different WBF distributions. We plot the
    significance of expected vector triplet signals (left: $S/B$,
    right: $S/\sqrt{B}$) against the theoretical uncertainty
    ...
    observables
    $x \in \{ q, \, p_{T,j_1}, \, p_{T,j_2}, \, p_{T,h} \}$. The lines
    guide the eye to the best-performing cut windows on these
    variables.}
...
$q$ as well as the leading tagging jet's $p_{T}$ define kinematic
...
\begin{equation}
  ...
  \underbrace{
    |\mat{$4$}^{\vphantom{*}} |^2
  }_{\displaystyle \ord{1\vphantom{/\Lambda^2}}}
  {} + {} \underbrace{ 
    2 \, \Real \, \mat{$4$}^* \mat{$6$}^{\vphantom{*}}
  }_{\displaystyle \ord{1/\Lambda^2}}
  {} + {} \underbrace{
    |\mat{$6$}^{\vphantom{*}}|^2 + 2 \, \Real \, \mat{$4$}^* \mat{$8$}^{\vphantom{*}}
}_{\displaystyle \ord{1/\Lambda^4}}
{} + \ord{1/\Lambda^6} \,,
\end{equation}
%
...
In a top-down perspective, \ie when the underlying physics is known,
the answer depends on the typical coupling strengths of the underlying
...
contain two couplings of the heavy field, $f_i \sim g_{\text{UV}}^2$,
and we have to compare the terms
%
\begin{equation}
...
\end{equation}
%
where $g_{\text{SM}}$ denotes a typical SM coupling. In strongly
coupled scenarios with $g_{\text{UV}} > g_{\text{SM}}$, we therefore
expect the squared dimension-six terms to dominate over the
dimension-eight contributions; it is then perfectly justified to
include the squared dimension-six term, but no dimension-eight
operators. In more weakly coupled models with
$g_{\text{UV}} \sim g_{\text{SM}}$, the two should approximately
contribute equally.  The size of the squared dimension-six terms can
then be seen as an estimate for the size of the missing higher orders
in the EFT expansion, and therefore as an indicator for the validity
of the EFT approach~\cite{Englert:2015hrx}.
...
scenarios. Similarly, using the size of the squared dimension-six
amplitude as a universal theory error in global fits introduces an
unnecessary theory dependence in the results.

Note also that the argument around \autoref{eq:validity_square_uv} relies
...
again with the example of WBF Higgs production in the vector triplet
model. The squared dimension-six terms improve the agreement with the
full model, and can be necessary to avoid negative rates at high jet
$p_T$.
...
$p_{T,j_1}$ distributions. We define a parameter point as excluded
...
\begin{equation}
  g_V = 1 \,, \qquad 
  c_H = c \,, \qquad 
  c_F = \frac {g_V^2}{2g^2} \, c \,, \qquad \text{and} \qquad
  c_{HHVV} = c^2 \,,
...
scale with $c^2/m_\xi^2$. Our perturbative calculation does not make
...
\begin{tabular}{ll c ccc}
  ...
  \midrule
  %
  singlet & on-shell ggF, WBF, $Vh$ && & & \largex \\
        & off-shell WBF  && & \brlargex & \largex \\
        & $hh$ && \largex & \largex & \largex \\
  %
  2HDM & on-shell ggF, WBF, $Vh$ && \brlargex & & \largex \\
...
We concluded our discussion of the vector triplet model
