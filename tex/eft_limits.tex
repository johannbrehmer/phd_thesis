...
\end{equation}
%
where $g$ is a typical coupling of the underlying theory and
...
simply the natural choices to describe a model. We thus expect that
...
\newparagraph
%
Two aspects complicate the decoupling in the 2HDM:
...
\eqref{eq:validity_2hdm_matching2} remain unchanged.\footnote{An
  ...
  matching prescription to demonstrate how much a simple redefinition
  of $\Lambda$ can change the results.} The two matching schemes can
exhibit significant differences in the 2HDM since the pseudoscalar
mass $m^2_{A^0} = m_{12}^2/(\sin\beta\cos\beta) -\lambda_5\,v^2$ does
not coincide with $\Lambda_{\text{default}}$ over wide ranges of the
parameter space.
...
reproduce the coupling patterns of the full model. We
