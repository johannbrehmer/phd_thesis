...
terms from $\ope{\phi,2}$.  Already at
...
their interferene in negligible. This allows us to define ``$W$-like''
...
\item isolate phase-space regions with interesting NP signatures with
  a cut $x > x_{\text{min}}$, and simultanously
...
\begin{equation}
  |\mat{EFT}|^2
  =
  \underbrace{ |\mat{$4$}|^2 }_{\ord{1}}
  + \underbrace{
    2 \, \text{Re} \mat{$4$}^* \mat{$6$}
  }_{\ord{1/\Lambda^2}}
  + \underbrace{
    |\mat{$6$}|^2 + 2 \, \text{Re} \mat{$4$}^* \mat{$8$}
}_{\ord{1/\Lambda^4}}
+ \ord{1/\Lambda^6} \,,
\end{equation}
%
where the subscripts in $\mat{$i$}$ denote the dimension of the
...
%
\begin{equation}
  |\mat{$d=6$}|^2 \sim \frac {g_{UV}^4} {\Lambda^4}
  %
  \quad \text{vs.} \quad
  %
  2 \, \text{Re} \mat{$d=4$}^* \mat{$d=8$} \sim \frac {g_{SM}^2 g_{UV}^2} {\Lambda^4} \,,
...
question of whether the squared dimension-six amplitudes shold be
...
partners, and a heavy vector triplet, focussing on parameter ranges
...
the full theory is not unambigous. First, instead of setting the
...
exceptions that confirm the rule are summarized in
...
Focussing on Higgs production in weak boson fusion in the vector
