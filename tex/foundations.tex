...
%
\begin{equation}
  \lgr{EFT} = (\text{kinetic and mass terms}) + \sum_i \frac {c_i} {\Lambda^{D_i - d}} \, \ope{i} \,.
...
Higgs couplings induced by the dimension-six operators of
Eqs.~\eqref{eq:foundations_operators_even} and
\eqref{eq:foundations_operators_odd}, some of which were calculated in
Refs.~\cite{Corbett:2012ja, Corbett_thesis} and cross-checked by
us. Here we expand non-linear effects from the field redefinition in
Eq.~\eqref{eq:foundations_ophi2_rescaling} to $\ord{1/\Lambda^2}$.

First, the Higgs-gauge interactions read
%
\begin{align}
  \lgr{EFT} &\supset
  \frac {f_{GG} v} {\Lambda^2} \, h G_{\mu \nu}^a G^{\mu \nu \, a}
  - \frac {g^2 v s^2 (f_{WW} + f_{BB}) } {4 \Lambda^2}  \, h A_{\mu \nu} A^{\mu \nu}
  + \dots
\end{align}
%
\comment{To here. Continue translating Eqs.~(15) and (16) of Power to
  the Data to here, and check with the EFT to its Limits appendix.}
