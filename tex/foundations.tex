...
contributions from the bottom. The total cross section for
...
Yukawa coupling ($t \overbar{t} h$ production is only sensitive to the
...
$gg \to b \overbar{b}$ background. WBF and $Vh$ production provide handles
...
limits also rely on specific model assumptions.
%
%The top Yukawa
...
degenercay~\cite{Corbett:2015ksa}.
%
...
acceptable level, this new physics should reside at energy scales not
...
Higgs boson or other (pseudo-)scalars in an extended Higgs sector. Such `Higgs
...
properties of effective theories. In this simple scenario we ignore that the full
...
freedom become irrelevant. At this \emph{matching scale} an effective
...
effective coupling $G_F$ is suppressed by $\Lambda^2 = m_W^2$.
...
in $1/\Lambda$, only a finite number of divergencies appears and can
...
list before using the equations of motion to define a non-redundant
basis. We classify the operators based on their field content and on
...
\begin{align}
  ...
  %
  g_{h\gamma\gamma} &= - \frac {g^2 v s_W^2 (f_{WW} + f_{BB}) } {4 \Lambda^2} \left(1 +  \frac{v^2  f_{\phi,2} } {\Lambda^2} \right)^{-1/2} \,, \notag \\
  %
  ...
  %
  g^{(4)}_{hWW} &= - \frac {g^2 v f_{W\widetilde{W}}} {4 \Lambda^2}  \left(1 +  \frac{v^2  f_{\phi,2} } {\Lambda^2} \right)^{-1/2} \,, \quad \text{and} \notag \\
  %
...
%
\begin{equation}
  g_{hxx} = \kappa_x \, g_{hxx}^{\text{SM}} \equiv (1 + \Delta_x) \, g_{hxx}^{\text{SM}} \,.
