...
$k_E^2 = (k^0_E)^2 + \boldsymbol{k}^2 = - k^2$, we find\footnote{This
...
%
\begin{align}
  T_3(p,q) &= \frac {-\im} {16 \pi^2} \, \int_0^1 \!\! \diff x \int_0^{1-x} \!\! \diff y \;
   \Bigl[ M^2 - (1 - x - y) (p+q)^2 - y p^2 \notag \\
  &\phantom{=} \qqqquad \qqquad+ ((1-x) p  + (1 - x - y) q )^2 \Bigr] \,.
\end{align}
%
There is a full solution for this integral in the
literature~\cite{tHooft:1978jhc, Denner:1991kt}, but the result is
extremely ugly\comment{Just like the dear proof reader\dots}.

Instead, we recognize that $p$ and $q$ correspond to momenta of the
light fields, and just evaluate the leading term in an expansion in
$1/M^2$.\comment{Need to sharpen this argument, ideally without
  identifying the momenta! Maybe along these lines: we know that the
  full integral is finite from the literature, we now show that the
  leading term in $1/M$ is finite, so the rest term is also
  finite. Still have to argue that the rest term does not develop
  powers of $M$ in the nominator\dots} We find
%
\begin{equation}
  T_3(p,q) = \frac {-\im} {16 \pi^2 \,M^2} + \ord{1/M^4} \,,
\end{equation}
%
and therefore
%
