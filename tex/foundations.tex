...
% %
% \begin{align}
%   \lgr{} &\supset \frac {c_{1 \, ijkl} } {\Lambda^2}  \left( \bar{\psi}_i \psi_j \right) \left( \bar{\psi}_k \psi_l \right)
%                      + \frac {c_{2 \, ijkl} } {\Lambda^2}  \left( \bar{\psi}_i  \gamma_5 \psi_j \right) \left( \bar{\psi}_k \psi_l \right) \notag \\
%            &\quad + \frac {c_{3 \, ijkl} } {\Lambda^2}  \left( \bar{\psi}_i  \gamma_5 \psi_j \right) \left( \bar{\psi}_k \gamma_5 \psi_l \right)
%                      + \frac {c_{4 \, ijkl} } {\Lambda^2}  \left( \bar{\psi}_i  \gamma_\mu \psi_j \right) \left( \bar{\psi}_k \gamma^\mu \psi_l \right) \notag \\
%           &\quad  + \frac {c_{5 \, ijkl} } {\Lambda^2}  \left( \bar{\psi}_i  \gamma_5 \gamma_\mu \psi_j \right) \left( \bar{\psi}_k \gamma^\mu \psi_l \right)
%                      + \frac {c_{6 \, ijkl} } {\Lambda^2}  \left( \bar{\psi}_i  \gamma_5 \gamma_\mu \psi_j \right) \left( \bar{\psi}_k \gamma_5 \gamma^\mu \psi_l \right) \notag \\
%          &\quad  + \frac {c_{7 \, ijkl} } {\Lambda^2}  \left( \bar{\psi}_i  \gamma_\mu \gamma_\nu \psi_j \right) \left( \bar{\psi}_k \gamma^\mu \gamma^\nu \psi_l \right) \,,
% \end{align}
% %
% Here some entries of the Wilson coefficient matrices $c_1$ to $c_7$
...
% violation it turns out that the $c_5$ and $c_6$ coefficients are equal
...
\begin{equation}
  ...
  % + \frac {c_3} {\Lambda^3} \phi_v^\dagger \phi_v v^\alpha \partial_\alpha (F_{\mu \nu} F^{\mu \nu})
  + \ord{1/\Lambda^4} \,,
\end{equation}
%
...
extremely ugly. \comment{Just like the dear proof
  reader\dots} Fortunately, we do not need it. Instead, we expand the
integrand in powers of $1/M^2$. We will only calculate the leading
term at $\ord{1/M^2}$. We will show that it produces a finite result
as well, so the rest term at $\ord{1/M^4}$ also has to be finite. Even
more, we can argue that the rest term at $\ord{1/M^4}$ has to vanish:
the coefficient at a given order $1/M^k$ in this expansion is an
integral without any mass scales, and has to lead to a result of mass
dimension $k-2$. Only $k = 2$ can give a non-zero and finite result,
all higher orders therefore have to vanish.
...
There are no dimension-5 operators,\footnote{After adding right-handed
  ...
  is entirely irrelevant for the physics we will discuss.} so the
leading effects in our EFT are expected to come from dimension-6
operators:
%
\begin{equation}
...
At this order, there are 59 independent types of operators, not
counting flavour structures and Hermitian
conjugation~\cite{Grzadkowski:2010es}. Counting all possible flavour
structures, there are 2499 distinct operators. Fortunately, in
...
operators. Three different conventions have become popular: in
addition to the ``Warsaw'' basis~\cite{Grzadkowski:2010es}, there is
the SILH convention~\cite{Giudice:2007fh} and the HISZ
basis~\cite{Hagiwara:1993ck}. For a comparison of and conversion
between these bases see Ref.~\cite{Falkowski:2015wza}\comment{More
  references?}. Throughout this thesis we use the basis developed
in~Refs.~\cite{Corbett:2012ja, Corbett_thesis}, which is strongly based
on the HISZ basis and now widely used in global
fits~\cite{Corbett:2015ksa, Butter:2016cvz}.

$CP$ transformations, the combination of charge conjugation and parity
inversion, are an approximate symmetry of the SM that is only violated
by the complex phase of the CKM matrix. In addition, there are rather
tight bounds on $CP$ violation in many processes. This motivates many
analyses to restrict their set of operators to the $CP$-conserving
ones. On the other hand, new sources of $CP$ violation are needed to
explain the matter-antimatter asymmetry in the universe, and their
effects at low energies would be visible as $CP$-violating effective
operators. Both types of operators will be analysed in this thesis.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{$CP$-even operators}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{table}
  ...
  \begin{tabular}{r @{${} = {}$} l @{\hspace*{0.8cm}} r @{${} = {}$} l } 
    ...
    \bottomrule
  \end{tabular}
  \caption[$CP$-even bosonic operators]{Bosonic $CP$-conserving
...
Higgs physics, following~\cite{Corbett:2012ja, Corbett_thesis}.  In
Table~\ref{tbl:foundations_operators_bosonic_even} we list the bosonic
ones, Table~\ref{tbl:foundations_operators_fermionic_even} gives the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{$CP$-odd operators}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{table}
  ...
  \begin{tabular}{r @{${} = {}$} l @{\hspace*{0.8cm}} r @{${} = {}$} l} 
    ...
    \bottomrule
  \end{tabular}
  \caption[$CP$-odd bosonic operators]{Bosonic $CP$-violating
...
$CP$-violating operators. We only list the bosonic operators relevant
for Higgs physics~\cite{Gavela:2014vra, Hankele:2006ma}, see
Table~\ref{tbl:foundations_operators_bosonic_odd}. They involve the
dual field strength tensors 
%
\comment{Why is
  $(\phi^\dagger \overleftrightarrow{D^2} \phi ) (\phisq)$ $CP$-odd?
  The Warsaw paper seems to disagree\dots} \comment{To here}
%
\begin{equation}
...
% \subsubsection{$\ope{t}$: $\phisq$ physics}

% Many Operators that consist of an SM dimension-4 operator $\ope{\text{dim-4}}$ multiplied with $\phisq$ have a straightforward phenomenology. Ignoring the Goldstones, we have
% \begin{align}
%   \lgr{EFT} &\supset c_{\text{dim-4}} \ope{\text{dim-4}} + \frac {f} {\Lambda^2} \, (\phisq) \, \ope{\text{dim-4}} \notag \\ 
%   {} &= c_{\text{dim-4}} \ope{\text{dim-4}} +   \frac {f} {\Lambda^2} \,  \frac {v^2 + 2 v h + h^2} {2} \ope{\text{dim-4}} \notag \\
%   {} &= \left(c_{\text{dim-4}} + \frac {v^2} 2 \right) \ope{\text{dim-4}} + \frac {f v} {2 \Lambda^2} \,  h \ope{\text{dim-4}} + \ord{h^2} \,.
% \end{align}
% The first term is a rescaling of $c_{\text{dim-4}}$, which is a free coupling of the SM. Such a rescaling will only be observable once we compare this coupling between different scales, i.\,e.\ in the RG flow of the theory. The second term will rescale the interactions of Higgs bosons with the other SM particles. The $h^2$ terms are for now less important as Higgs pair production will not be experimentally accessible too soon.

% For example, $\ope{\tau}$ will lead to a rescaling of the tau Yukawa couplings:
% \begin{align}
%   \lgr{EFT} &\supset \frac {f_{\tau} v} {2 \Lambda^2} \,  h \, \bar{F}_{L} \phi f_R  \notag \\
%   {} &= \dots
% \end{align}
...
Second, $\ope{W}$ changes the shape of distributions. One example is
...
One alternative is to dress the SM Lagrangian with form factors, as
