
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Header
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{tex/header.tex}
\input{tex/definitions.tex}

\title{New Ideas for Effective Higgs Measurements}
\subtitle{PhD Thesis in Progress}
\author{Johann Brehmer}
\date{Version from \today}

\begin{document}

\begin{fmffile}{diagrams}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Cover (only for digital version)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

{
\renewcommand*{\thepage}{cover.\arabic{page}}
\includepdf{cover/cover.pdf}
\cleardoubleemptypage
}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Front matter
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\frontmatter 
\pagestyle{plain}
\KOMAoptions{cleardoublepage=plain}

% Title and dedication
\input{tex/title.tex}

% Abstracts
\input{tex/abstract.tex}

% Table of Contents
\microtypesetup{protrusion=false} % disables protrusion for ToC 
%\KOMAoptions{open=left}
\tableofcontents 
%\KOMAoptions{open=right}
\microtypesetup{protrusion=true} % enables protrusion again

% Preface
\input{tex/preface.tex}

\thispagestyle{plain}
\cleardoubleemptypage


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Content
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\mainmatter 
\pagestyle{headings}
\KOMAoptions{cleardoublepage=plain}

\input{tex/introduction.tex}
\input{tex/foundations.tex}

\input{tex/eft_limits.tex}
\input{tex/information_geometry.tex}

\input{tex/conclusions.tex}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Back matter
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Appendix formatting
\backmatter 
%\KOMAoptions{cleardoublepage=empty}

\let\oldsection\thesection
\renewcommand{\thesection}{A.\oldsection}
\let\oldequation\theequation
\renewcommand{\theequation}{A.\oldequation}
\let\oldfigure\thefigure
\renewcommand{\thefigure}{A.\oldfigure}
\let\oldtable\thetable
\renewcommand{\thetable}{A.\oldtable}

% Acknowledgements
\input{tex/acknowledgements}

% \appendix
\input{tex/appendix}

% References
\bibliography{bib/own_work,bib/experiment,bib/theory,bib/eft,bib/pheno,bib/statistics,bib/tools,bib/quotes}

% List of figures and tables?
% \listoffigures
% \addcontentsline{toc}{chapter}{List of Figures}
% \listoftables 
% \addcontentsline{toc}{chapter}{List of Tables}

% Glossary
% \input{tex/glossary.tex}





%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% That's it.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\end{fmffile}

\end{document}